...
void test_compressed_format_course_speed()
{
    ...
    assert_true(strncmp(buffer, "/5L!!<*e7>", 10) == 0, "format course/speed prefix matches");
    assert_true(buffer[10] != ' ', "course byte is not space");
    assert_true(buffer[11] != ' ', "speed byte is not space");
    assert_true(buffer[12] == '[', "T byte is correct for GPS_FIX_CURRENT/NMEA_RMC/COMPRESS_SOFTWARE");
}
...
void test_compressed_format_radio_range()
{
    ...
    assert_true(strncmp(buffer, "/5L!!<*e7>", 10) == 0, "format radio range prefix matches");
    /* cs1 should be '{' for radio range */
    assert_true(buffer[10] == '{', "cs1 is '{' for radio range");
    assert_true(buffer[11] != ' ', "cs2 is not space for valid range");
}
...
void test_compressed_format_altitude()
{
    ...
    assert_true(strncmp(buffer, "/5L!!<*e7>", 10) == 0, "format altitude prefix matches");
    assert_true(buffer[10] != ' ', "cs1 is not space for valid altitude");
    assert_true(buffer[11] != ' ', "cs2 has value even though altitude only uses cs1");
}
...
void test_compressed_format_nmea_sources()
{
    ...
    assert_true(buffer[12] == ')', "NMEA_GLL T byte is correct");

    /* NMEA_GGA = 2 */
    memset(buffer, 0, sizeof(buffer));
    pos.nmea_source = NMEA_GGA;
    position_format_compressed(buffer, &pos);
    assert_true(buffer[12] == '1', "NMEA_GGA T byte is correct");

    /* NMEA_RMC = 3 */
    memset(buffer, 0, sizeof(buffer));
    pos.nmea_source = NMEA_RMC;
    position_format_compressed(buffer, &pos);
    assert_true(buffer[12] == '9', "NMEA_RMC T byte is correct");
}
...
void test_compressed_format_compression_origins()
{
    ...
    position_format_compressed(buffer, &pos);
    assert_true(buffer[12] == '\"', "COMPRESS_TNC_BTEXT T byte is correct");

    /* COMPRESS_KPC3 = 4 */
    memset(buffer, 0, sizeof(buffer));
    pos.compression_origin = COMPRESS_KPC3;
    position_format_compressed(buffer, &pos);
    assert_true(buffer[12] == '%', "COMPRESS_KPC3 T byte is correct");

    /* COMPRESS_DIGIPEATER = 7 */
    memset(buffer, 0, sizeof(buffer));
    pos.compression_origin = COMPRESS_DIGIPEATER;
    position_format_compressed(buffer, &pos);
    assert_true(buffer[12] == '(', "COMPRESS_DIGIPEATER T byte is correct");
}
...
void test_compressed_format_gps_fix_status()
{
    ...
    position_format_compressed(buffer, &pos);
    assert_true(buffer[11] == ' ', "No extra data bytes are space");
    assert_true((buffer[12] - 33) & 0x20, "GPS fix bit is set for CURRENT");
}
